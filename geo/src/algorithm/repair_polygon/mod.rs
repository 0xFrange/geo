...
fn label_faces<T: SpadeTriangulationFloat>(
    ...
    // whose face() is an inner face on the convex hull boundary.
    for edge in cdt.directed_edges() {
        ...
        if let Some(inner) = opposite.as_inner() {
            ...
            if visited.insert(handle) {
                let is_interior = edge.as_undirected().is_constraint_edge();
                if is_interior {
    ...
    while let Some((face_handle, face_is_interior)) = queue.pop_front() {
        ...
        for edge in face.adjacent_edges() {
            ...
            let inner_neighbour = neighbour.as_inner().expect("not outer, so must be inner");
            let n_handle = inner_neighbour.fix();
            if !visited.insert(n_handle) {
                continue;
            }
            let crosses_constraint = edge.as_undirected().is_constraint_edge();
...
fn trace_rings<T: SpadeTriangulationFloat>(
    ...
    for &start_fix in &boundary_edges {
        if used.contains(&start_fix) {
            continue;
        }

        let mut ring_coords: Vec<Coord<T>> = Vec::new();
        let mut current_fix = start_fix;
